...
// ─── Users ORM ────────────────────────────────────────────────────────────
export const Users = {
  async create({ email, passwordHash, passwordSalt, displayName, role = 'user' }) {
    const res = await query(
      `INSERT INTO users (email, password_hash, password_salt, display_name, role)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, email, display_name, role, created_at`,
      [email.toLowerCase(), passwordHash, passwordSalt, displayName || null, role]
    );
    return res.rows[0];
  },
  ...
  async findByEmail(email) {
    const res = await query(
      `SELECT id, email, password_hash, password_salt, display_name, role, is_active, last_login_at, created_at
  ...
  async existsByEmail(email) {
    const res = await query('SELECT 1 FROM users WHERE email = $1', [email.toLowerCase()]);
