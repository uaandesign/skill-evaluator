...
/**
 * 主入口：对一个上传的 skill 跑评估标准
 *
 * @param {Buffer|string} skillInput  zip Buffer 或 已解压目录路径
 * @param {Object} options
 * @param {string[]} [options.standardKeys]    可选：只跑指定 key 的 DB 标准
 * @param {Array}    [options.inlineStandards] 可选：直接传入完整标准定义，覆盖 DB
 *   每项: { standard_key, display_name?, rubric_version?, total_score?,
 *          skill_md_content, script_filename, script_content, references_md? }
 *
 * 优先级：inlineStandards > DB 启用标准
 * 任一来源都没有时：抛错让调用方提示用户配置
 *
...
export async function evaluateSkillAgainstAllStandards(skillInput, options = {}) {
  ...
  // ─── 决定使用哪些评估标准 ────────────────────────────────────────────
  // 优先级：用户在前端 inline 传入的标准（如 evalStandards localStorage 上传）
  //         > DB 启用的标准（包含内置 seed 的标准）
  let fullStandards = [];

  if (options.inlineStandards && options.inlineStandards.length > 0) {
    // inline 模式：直接使用前端传入的标准
    fullStandards = options.inlineStandards.map((s, idx) => ({
      id: s.id || `inline-${idx}`,
      standard_key:    s.standard_key || `inline-${idx}`,
      display_name:    s.display_name || s.standard_key || `Inline ${idx}`,
      description:     s.description || '',
      rubric_version:  s.rubric_version || null,
      total_score:     s.total_score || 100,
      skill_md_content: s.skill_md_content,
      script_filename:  s.script_filename || 'evaluate.py',
      script_content:   s.script_content,
      references_md:    s.references_md || null,
      is_builtin:       false,
      _source:          'inline',
    }));
  } else {
    // DB 模式：读启用的标准
    const standardKeys = options.standardKeys;
  ...
  if (fullStandards.length === 0) {
    safeRm(tmpRoot);
    throw new Error('未找到启用的评估标准。请先在配置中心上传或启用至少一个标准。');
  }
